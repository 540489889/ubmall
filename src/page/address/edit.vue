...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getInitData(){
        let id = parseInt(this.$route.query.aid)
        this.http.post(this.ports.address.detail,{aid: id}, res =>{
          ...
          if(res.success){
            ...
            this.is_default = data.status
            this.name = data.name
            this.tel = data.phone
            this.address = data.detailed
...
//            this.$router.push({ path: '/address/index' });
      ...
      addAddress(){
        ...
        params.detailed = this.address
        params.district = this.area
        params.default = this.is_default //0：不是默认，1：默认
        this.http.post(this.ports.address.revise,params, res =>{
          this.$store.commit('changeLoading',false)
          console.log(res)
          if(res.success){
            let data = res.data
