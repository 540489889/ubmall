      ...
      <div class="goods-infor flex-box">
        ...
        <div class="right-1 box-1">
          ...
          <div class="top flex-box">
            {{(defaultGoods.price*number).toFixed(2)}}<span>BCAC</span>
            <!--<div class="mb-ui flex-box">-->
              <!--<div class="left box-1">会员价</div>-->
              <!--<div class="right box-1">{{Number(goods.price_member)}}</div>-->
            <!--</div>-->
          </div>
          <p>库存：{{defaultGoods.stock}}</p>
        </div>
      </div>
      <div class="specs-infor">
        <h2></h2>
        <ul>
          <li @click="checkSpecs($event,it,index)" :class="index==0?'cube-checker-item_active':''"  v-for="(it,index) in goods.spec_id" class="cube-checker-item">
            {{it.name}}
          </li>
        </ul>
        <div class="nummber-infor flex-box">
          ...
          <div class="right flex-box">
            ...
            <cube-input :min="1" :max="10" type="number" @blur="blurInput" @change="handleNumChange(data.item)" v-model="number"></cube-input>
    ...
    methods: {
      ...
      createOrder(){
        this.$router.push({
          path: '/order/orders',
          query: {
            checkGoods:this.goods.id+':'+this.number+':'+0+':'+this.defaultGoods.spec_id,
          }
        })
      },
      ...
      addCart(){
        let params = {}
        params.pid = this.goods.id
        params.spec_id = this.defaultGoods.spec_id
        params.number = this.number
        this.http.post(this.ports.cart.addCart,params, res =>{
          if(res.success){
            let data = res.data
            this.$emit('changeAddCart',this.number,data.is_add)
//            this.$emit('changeSelected',false)
      ...
      //默认规格
      defaulCheck(){
        this.defaultGoods = this.goods.spec_id[0]
      },
      //规格选择
...
<style lang="less" scoped>
  ...
  .goodsSelected{
    ...
    .closeBtn{
      ...
      width:32px;
      height:32px;
      background:url(../../../assets/icon/close-ico.png) no-repeat center;
      background-size:100%;
      color:#ccc;
      font-size:45px;
      font-weight: bold;
